...
// 引入模块
const Cos_service = require('cos-nodejs-sdk-v5');
const fs = require('fs');
const path = require('path');
const util = require('util');
const config = require('../config/db');

let cos = new Cos_service({
  // 必选参数
...
class COSService {
   ...
   * @return {Promise<void>}
   */
  async sliceUploadFile(filepath, filename, dirPath) {
    const sliceUploadFile = util.promisify(cos.sliceUploadFile.bind(cos));
    // 调用方法
    let result = await sliceUploadFile({
      Bucket: config.COS.Bucket,  /* 必须 */ // Bucket 格式：test-1250000000
      Region: config.COS.Region,
      Key: dirPath + filename,    //filename, /* 必须 */
      FilePath: filepath,            //filepath, /* 必须 */
    });

    // if (err) {
    //   console.error('sliceUploadFile ===', err);
    // }
    // console.log('sliceUploadFile data ==', result);
    return result;
  }
  ...
  async deleteObject(filename) {
    let {ctx, config} = this;
    let [err, data] = await ctx.helper.toClarify(cos.deleteObject, cos)({
